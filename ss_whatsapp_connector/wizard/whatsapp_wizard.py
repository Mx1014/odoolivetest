...
class WhatsappSendMessage(models.TransientModel):
    ...
    def _domain_template_id(self):
        if self.env.context.get('source_model'):
            return [('model_id.model', '=', self.env.context.get('source_model'))]
        return []
    ...
    template_id = fields.Many2one('mail.template', 'Use template', index=True, domain=_domain_template_id)
    ...
    def send_message(self):
        #     self.env['whatsapp.api'].send_media(self.message, self.attachment_ids, ids, self.mobile_number)
        # else:
        if self._context.get('active_model') == 'sale.coupon':
            ids = self._context.get('active_ids')
            coup = self.env['sale.coupon'].search([('id', 'in', ids)])
            coup.coupon_send = True
