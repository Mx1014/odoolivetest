...
class AccountBahrainVat(models.AbstractModel):
    _name = "account.report.bh.vat"
    ...
    #     templates = super(AccountBahrainVat, self)._get_templates()
    #     templates['main_template'] = 'l10n_bh.action_account_bahrain_vat'
    #     return templates
    ...
    def _get_options(self, previous_options=None):
        ...
        return super(AccountBahrainVat, self)._get_options(previous_options)

    def _get_reports_buttons(self):
        res = super(AccountBahrainVat, self)._get_reports_buttons()
    ...
    def _compute_vat_closing_entry(self, raise_on_empty):
        ...
        self.env['account.move.line'].flush(
            ['account_id', 'debit', 'credit', 'move_id', 'tax_line_id', 'date', 'tax_exigible', 'company_id',
             'display_type'])
    ...
    def _find_create_move(self, date_from, date_to, company_id):
        move = self.env['account.move'].search(
            [('is_tax_closing', '=', True), ('date', '>=', date_from), ('date', '<=', date_to)], limit=1,
            order='date desc')
        if len(move):
            return move
        else:
    ...
    def _generate_tax_closing_entry(self, options, move=False, raise_on_empty=False):
        ...
        if len(line_ids_vals):
            ...
            if raise_on_empty:
                ...
                msg = _(
                    'It seems that you have no entries to post, are you sure you correctly configured the accounts on your tax groups?')
    ...
    def _get_columns_name(self, options):
        ...
        if options.get('tax_grids'):
            columns_header.append({'name': '%s \n %s' % (_('Amount'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            columns_header.append({'name': '%s \n %s' % (_('Adjustment'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            columns_header.append({'name': '%s \n %s' % (_('VAT Amount'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            ...
            columns_header += [{'name': '%s \n %s' % (_('NET'), self.format_date(options)), 'class': 'number',
                                'style': 'white-space: pre;'}, {'name': _('TAX'), 'class': 'number'}]
            if options.get('comparison') and options['comparison'].get('periods'):
                for p in options['comparison']['periods']:
                    columns_header += [{'name': '%s \n %s' % (_('NET'), p.get('string')), 'class': 'number',
                                        'style': 'white-space: pre;'}, {'name': _('TAX'), 'class': 'number'}]
    ...
    def _set_context(self, options):
        ctx = super(AccountBahrainVat, self)._set_context(options)
    ...
    def _get_type_tax_use_string(self, value):
        return \
        [option[1] for option in self.env['account.tax']._fields['type_tax_use'].selection if option[0] == value][0]
    ...
    def _get_lines_by_grid(self, options, line_id, grids):
        ...
        report_lines = self.env['account.tax.report.line'].search(
            [('country_id', '=', country.id), ('parent_id', '=', False)])
        ...
        for (index, total_line) in deferred_total_lines:
            ...
            total_period_number = 1 + (
                        options['comparison'].get('periods') and options['comparison']['number_period'] or 0)
            lines[index] = self._build_total_line(total_line, balances_by_code, hierarchy_level, total_period_number,
                                                  options)
    ...
    def _postprocess_lines(self, lines, options):
        ...
        def assign_active_section(col_nber):
            ...
            line_to_assign['columns'] = [
                {'name': self.format_value(balance), 'style': 'white-space:nowrap;', 'balance': balance} for balance in
                total_balance_col]
            ...
            if line['id'] == 'deferred_total':
                ...
                if active_sections_stack:
                    for active_section in active_sections_stack:
                        ...
                        totals_by_line[active_section['id']] = line_balances if not rslt_balances else [
                            line_balances[i] + rslt_balances[i] for i in range(0, len(rslt_balances))]
    ...
    def compute_check(self, lines, options):
        ...
        for i, calc in enumerate(self.get_checks_to_perform(mapping)):
            if calc[1]:
                if isinstance(calc[1], float):
                    ...
                    value = calc[1]
                controls.append({'name': calc[0], 'id': 'control_' + str(i),
                                 'columns': [{'name': value, 'style': 'white-space:nowrap;', 'balance': calc[1]}]})
        ...
        if controls:
            lines.extend([{'id': 'section_control', 'name': _('Controls failed'), 'unfoldable': False,
                           'columns': [{'name': '', 'style': 'white-space:nowrap;', 'balance': ''}] * col_nber,
                           'level': 0, 'line_code': False}] + controls)
            options[
                'tax_report_control_error'] = "<table width='100%'><tr><th>Control</th><th>Difference</th></tr>{}</table>".format(
                "".join(html_lines))
    ...
    def _build_total_line(self, report_line, balances_by_code, hierarchy_level, number_periods, options):
        ...
        for period_index in range(0, number_periods):
            ...
            period_date_from = (period_index == 0) and options['date']['date_from'] or \
                               options['comparison']['periods'][period_index - 1]['date_from']
            period_date_to = (period_index == 0) and options['date']['date_to'] or \
                             options['comparison']['periods'][period_index - 1]['date_to']

            eval_dict = self._get_total_line_eval_dict(period_balances_by_code, period_date_from, period_date_to,
                                                       options)
            period_total = safe_eval(report_line.formula, eval_dict)
            columns.append(
                {'name': self.format_value(period_total), 'style': 'white-space:nowrap;', 'balance': period_total})
            ...
            for key, tax in sorted(groups[tp].items(), key=lambda k: k[1]['obj'].sequence):
                if tax['show']:
                    ...
                    for period in tax['periods']:
                        columns += [{'name': self.format_value(period['net'] * sign), 'style': 'white-space:nowrap;'},
                                    {'name': self.format_value(period['tax'] * sign), 'style': 'white-space:nowrap;'}]
                    lines.append({
                        'id': tax['obj'].id,
                    ...
                    for child in tax.get('children', []):
                        ...
                        for period in child['periods']:
                            columns += [
                                {'name': self.format_value(period['net'] * sign), 'style': 'white-space:nowrap;'},
                                {'name': self.format_value(period['tax'] * sign), 'style': 'white-space:nowrap;'}]
                        lines.append({
                            'id': child['obj'].id,
    ...
    def _compute_tax_report_data(self, options):
        ...
        search_domain = options.get('tax_grids') and [('country_id', '=', self.env.company.country_id.id)] or [
