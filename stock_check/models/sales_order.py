...
class SaleOrderLine(models.Model):
    ...
    # x_forecast_availability = fields.Float('Forecast Availability', compute='_compute_forecast_information', compute_sudo=True)
    # x_forecast_expected_date = fields.Datetime('Forecasted Expected date', compute='_compute_forecast_information',
    #                                          compute_sudo=True)
    x_related_stock_quant_ids = fields.One2many(related='product_id.stock_quant_ids', string='Related Stock Quant')

    @api.model
    def get_stock_location_vals(self, res_id):
        print(res_id[0])
        res = []
        disc = {}
        for record in self.env['stock.quant'].search([('id', 'in', res_id[0]), ('location_id.usage', '=', 'internal')]):
            res.append(record.location_id.complete_name)
            res.append(record.x_free_qty)
            disc[record.location_id.complete_name] = record.x_free_qty
        return disc

    # @api.depends('product_id')
    # def _compute_forecast_information(self):
    #     """ Compute forecasted information of the related product by warehouse."""
    #     self.x_forecast_availability = False
    #     self.x_forecast_expected_date = False
    #
    #     not_product_moves = self.filtered(lambda line: line.product_id.type != 'product')
    #     for line in not_product_moves:
    #         line.x_forecast_availability = line.product_uom_qty
    #
    #     product_moves = (self - not_product_moves)
    #     warehouse_by_location = {loc: loc.get_warehouse() for loc in product_moves.product_id.location_id}
