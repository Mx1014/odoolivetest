...
from odoo.addons.stock_check.models.stock_rule import ProcurementException
from odoo.exceptions import UserError, ValidationError
from odoo.osv import expression
from odoo.tools import add, float_compare, frozendict, split_every, format_date, float_round, DEFAULT_SERVER_DATETIME_FORMAT
...
class StockWarehouseOrderpoint(models.Model):
    ...
    qty_to_order = fields.Float('To Order', readonly=False, store=True, compute='_compute_qty_to_order')
    ...
    def action_replenish(self):
        value = self._procure_orderpoint_confirm(company_id=self.env.company)
        notification = False
        if len(self) == 1:
    ...
    @api.depends('product_id', 'location_id', 'product_id.stock_move_ids', 'product_id.stock_move_ids.state', 'product_id.stock_move_ids.product_uom_qty', 'product_id.purchase_order_line_ids.state', 'product_id.purchase_order_line_ids')
    ...
    def _get_orderpoint_action(self):
        ...
        """
        action = self.env["ir.actions.act_window"].for_xml_id("stock_check", "action_orderpoint_replenish")
    ...
    def _get_orderpoint_domain(self, company_id=False):
        domain = [('company_id', '=', company_id.id)] if company_id else []
    ...
    #                 raise
    #         else:
    #             OrderPoint._post_process_scheduler()
    #             break
    #
    #         if use_new_cursor:
    #             cr.commit()
    ...
    def _get_replenishment_order_notification(self):
        self.ensure_one()
        order = self.env['purchase.order.line'].search([('orderpoint_id', 'in', self.ids), ('state', '=', 'draft'), ('partner_id', '=', self.supplier_id.name.id)], limit=1).order_id
        if order:
            action = self.env.ref('stock_check.action_rfq_form')
